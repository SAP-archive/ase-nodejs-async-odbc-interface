...
module.exports.connect = function (connectionString, options, cb) {
  var db;
...
Database.prototype.connect = function (connectionString, cb) {
  var self = this;
  ...
  self.odbc.createConnection(function (err, conn) {
    ...
    self.conn.connect(connectionString, function (err, result) {
      if (err) return cb(err);
...
Database.prototype.disconnect = function (cb) {
  ...
  self.queue.push(function (next) {
    ...
    self.conn.disconnect(function (err) {
      self.connected = false;
      delete self.conn;
...
Database.prototype.exec = function (sql, params, cb) {
  ...
  self.queue.push(function (next) {
    ...
    if (params) {
      self.conn.exec(sql, params, cbQuery);
    }
    else {
      self.conn.exec(sql, cbQuery);
    }
  });
};

Database.prototype.execResult = function (sql, params, cb) {
  var self = this;
...
Database.prototype.describe = function(obj, callback) {
  ...
  if (obj.table && obj.column) {
    ...
    self.tables(obj.database, obj.schema, null, null, callback);
  }
};
...
Pool.prototype.open = function (connectionString, callback) {
  ...
  if (self.availablePool[connectionString] && self.availablePool[connectionString].length) {
    ...
    db.connect(connectionString, function (error) {
    ...
    for (key in pools) {
      ...
      for (x = 0 ; x < connections.length; x ++) {
        (function (x) {
          ...
          connections[x].realClose = connections[x].close;
          console.log("Hi0");
          connections[x].close = function (){
