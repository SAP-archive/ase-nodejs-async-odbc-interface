...
NAN_METHOD(ODBCResult::Close) {
    ...
    Local<Function> cb;
    DEBUG_PRINTF("ODBCResult::Close1\n");

    if (info.Length() == 1 && info[0]->IsFunction()) {
        cb = Local<Function>::Cast(info[0]);
    }
    else {
        return Nan::ThrowTypeError("ODBCResult::Close(): 1  argument is required and it must be a callback function.");
    }
    DEBUG_PRINTF("ODBCResult::Close2\n");
    data->cb = new Nan::Callback(cb);
    DEBUG_PRINTF("ODBCResult::Close3\n");
    ...
    DEBUG_PRINTF("ODBCResult::Close after call1\n");
    objODBCResult->Ref();
    DEBUG_PRINTF("ODBCResult::Close after call1\n");
    info.GetReturnValue().Set(Nan::Undefined());
    DEBUG_PRINTF("ODBCResult::Close after call2\n");
}

void ODBCResult::UV_Close(uv_work_t* work_req) {
    DEBUG_PRINTF("ODBCResult::UV_Close\n");

    fetch_work_data* data = (fetch_work_data *)(work_req->data);

    data->result = SQLFreeStmt(data->objResult->m_hSTMT, SQL_CLOSE);
    DEBUG_PRINTF("ODBC return value %i \n", data->result);
}

void ODBCResult::UV_AfterClose(uv_work_t* work_req, int status)
{
    DEBUG_PRINTF("ODBCResult::UV_AfterClose\n");
    Nan::HandleScope scope;

    fetch_work_data* data = (fetch_work_data *)(work_req->data);
    DEBUG_PRINTF("ODBCResult::UV_AfterClose%i\n", data->objResult->m_hSTMT);

    ODBCResult* self = data->objResult->self();
    if (data->result == SQL_ERROR) {
        DEBUG_PRINTF("ODBCResult::UV_AfterClose In if\n");
    ...
    {
        DEBUG_PRINTF("ODBCResult::UV_AfterClose In else\n");
        Local<Value> info[1];
        info[0] = Nan::Null();
        ...
        DEBUG_PRINTF("ODBCResult::UV_AfterClose In else after call\n");
    }
...
NAN_METHOD(ODBCResult::MoreResultsSync) {
  ...
  if (ret == SQL_ERROR) {
    Local<Value> objError = ODBC::GetSQLError(SQL_HANDLE_STMT, result->m_hSTMT, (char *)"[node-odbc] Error in ODBCResult::MoreResultsSync");
